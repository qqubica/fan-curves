...
public class CurveEditor : FrameworkElement
{
    ...
    private int PtCount => _channel?.Points.Count ?? 0;

    private (double X, double Pct) Pt(int i) =>
    ...
    private Point ToScreen(double x, double pct)
    {
        var r = Plot;
        return new Point(
            r.Left + (x - TempMin) / (TempMax - TempMin) * r.Width,
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        var stairs = new List<Point> { ToScreen(TempMin, pts[0].Pct) };
        for (int i = 0; i < pts.Count; i++)
        {
            double bandEnd = i < pts.Count - 1 ? pts[i + 1].X : TempMax;
        ...
        // like every other live-thermal readout (dot, crosshair) — temperature is the
        // amber quantity.
        if (ShowRaw && _liveRaw is double raw)
        {
            ...
            var t = Label($"now {raw.ToString("0.0", CultureInfo.InvariantCulture)}°",
                RawTempLabelBrush);
            dc.DrawText(t, new Point(Math.Min(x + 6, Math.Max(r.Left, r.Right - t.Width)), r.Top + 2));
        }
        ...
        if (_liveEffective is double eff && !double.IsNaN(eff) && _liveOutput is double outPct)
        {
            ...
            dc.DrawLine(AmberHairPen, new Point(p.X, r.Bottom), p);
            dc.DrawLine(AmberHairPen, new Point(r.Left, p.Y), p);

            dc.DrawEllipse(AmberGlow, null, p, 16, 16);
            dc.DrawEllipse(AmberBrush, null, p, 4.2, 4.2);
        ...
        // with °/% chips over the axis labels (amber stays live-data-only).
        ...
        // reading out the °/% under the cursor.
        else if (_hoverPos is Point hp && !IsReadOnly)
        {
            var (hx, hpct) = FromScreen(hp);
            DrawWhiteCrosshair(dc, r, Label, Math.Round(hx), Math.Round(hpct),
        ...
        double x, double pct, Pen hairPen, Brush textBrush)
    {
        ...
        var tx = label(Inv($"{x:0.#}°"), textBrush, 10.5);
        var txPos = new Point(
            Math.Clamp(p.X - tx.Width / 2, r.Left, Math.Max(r.Left, r.Right - tx.Width)), r.Bottom + 8);
        dc.DrawRectangle(CardBrush, null,
            new Rect(txPos.X - 4, txPos.Y - 1, tx.Width + 8, tx.Height + 2));
    ...
    protected override void OnMouseLeftButtonDown(MouseButtonEventArgs e)
    {
        ...
        if (e.ClickCount == 2)
        {
            ...
            double x = Math.Round(rawX), pct = Math.Round(rawPct);
            // Keep every band at least 1° wide — the drag clamp assumes it.
            if (PtCount < 12 && HitTestPoint(pos) < 0
                && Enumerable.Range(0, PtCount).All(i => Math.Abs(Pt(i).X - x) >= 1))
            {
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        double minX = _dragIndex > 0 ? Pt(_dragIndex - 1).X + 1 : TempMin;
        double maxX = _dragIndex < PtCount - 1 ? Pt(_dragIndex + 1).X - 1 : TempMax;
        if (minX > maxX) return; // neighbours from an old profile can sit under 1° apart
        _channel.Points[_dragIndex] = new CurvePoint(
            Math.Clamp(Math.Round(x), minX, maxX), Math.Round(pct));
        InvalidateVisual();
    }
